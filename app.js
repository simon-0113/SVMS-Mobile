...
function renderSession() {
  ...
  if (!downloadButton) {
    ...
    downloadButton.addEventListener("click", () => {
      ...
      const payload = {
        ...buildPendingPayload(currentSession),
        user_name: userName,
        batch_id: `${localDateISO().replaceAll("-", "")}_${safeFilePart(userName)}_mobile`,
        uploaded_at: new Date().toISOString()
      };
      ...
      downloadJSON(
        payload,
        `${safeFilePart(userName)}_${localTimestampForFile()}.json`
      );
...
$("#completeSessionButton").addEventListener("click", async () => {
  ...
  const success = await uploadTeamPending(buildPendingPayload(session));
